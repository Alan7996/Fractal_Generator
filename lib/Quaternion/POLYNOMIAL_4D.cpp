...
// Read from .poly4d file
POLYNOMIAL_4D::POLYNOMIAL_4D(const string filename) {
    FILE* file = fopen(filename.c_str(), "rb");
    if (file == NULL) {
        cout << "Unable to read polynomial from file " << filename << endl;
        exit(1);
    }
    read(file);
    fclose(file);
